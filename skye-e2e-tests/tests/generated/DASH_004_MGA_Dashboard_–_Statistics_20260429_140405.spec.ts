...
test('DASH_004 - Validate Dashboard Statistics Cards and Data Table', async ({ page, skye, mga }) => {
    ...
    await test.step('Step 3: Verify "With You" card displays numeric value', async () => {
        const withYouCard = page.getByTestId(DASHBOARD_TEST_IDS.withYouCard);
        const valueElement = withYouCard.locator('h2').first();
    ...
    await test.step('Step 4: Verify "With You" card label contains expected text', async () => {
        const withYouCard = page.getByTestId(DASHBOARD_TEST_IDS.withYouCard);
        const labelElement = withYouCard.locator('h4').first();
        await expect(labelElement).toContainText(EXPECTED_LABELS.withYou);
    });
    ...
    await test.step('Step 6: Verify "With the Broker" card displays numeric value and correct label', async () => {
        ...
        const valueElement = withBrokerCard.locator('h2').first();
        await expect(valueElement).toBeVisible();
        const valueText = await valueElement.innerText();
        expect(valueText.trim()).toMatch(NUMERIC_PATTERN);
        
        const labelElement = withBrokerCard.locator('h4').first();
        await expect(labelElement).toContainText(EXPECTED_LABELS.withBroker);
    });
    ...
    await test.step('Step 8: Verify "With the Client" card displays numeric value and correct label', async () => {
        ...
        const valueElement = withClientCard.locator('h2').first();
        await expect(valueElement).toBeVisible();
        const valueText = await valueElement.innerText();
        expect(valueText.trim()).toMatch(NUMERIC_PATTERN);
        
        const labelElement = withClientCard.locator('h4').first();
        await expect(labelElement).toContainText(EXPECTED_LABELS.withClient);
    });
